...
class FJSPEnv_test():
    ...
    def step(self, actions):
        ...
        # --- Add Predecessor Machine ID feature (index 6) ---
        job_ops = env.get_job(operation.job_id).operations
        predecessor_op = None
        # Find the current operation's index in the job's ordered operations list
        current_op_index_in_job = -1
        for i, op in enumerate(job_ops):
            if op.operation_id == operation.operation_id:
                current_op_index_in_job = i
                break

        if current_op_index_in_job > 0:
            predecessor_op = job_ops[current_op_index_in_job - 1]

        if predecessor_op and predecessor_op.scheduled_machine is not None:
            self.JSM_feat_opes_batch[self.batch_idxes, 6, ope_idx] = predecessor_op.scheduled_machine
        else:
            # No predecessor or predecessor not yet scheduled, set to a default (e.g., -1)
            self.JSM_feat_opes_batch[self.batch_idxes, 6, ope_idx] = -1 # Using -1 to indicate no predecessor machine
        ...
        for each_mach in self.JSP_instance.machines:
            raw_workload = sum([ope_on_mach.scheduled_duration for ope_on_mach in each_mach.scheduled_operations])
            cur_time = self.JSM_time
            # Utilization (original feature at index 2)
            self.JSM_feat_mas_batch[self.batch_idxes, 2, each_mach.machine_id] = raw_workload / (cur_time + 1e-9) if cur_time > 0 else 0
            # Next available time (original feature at index 1)
            self.JSM_feat_mas_batch[self.batch_idxes, 1, each_mach.machine_id] = each_mach.next_available_time
            # Raw Workload (new feature at index 3)
            self.JSM_feat_mas_batch[self.batch_idxes, 3, each_mach.machine_id] = raw_workload
        ...
        # --- Update Average Eligible Machine Load (index 7) ---
        for op_to_update in self.JSP_instance.operations:
            if op_to_update.scheduled_start_time is None:
                eligible_machine_ids = op_to_update.optional_machines_id
                eligible_machine_loads = []
                for mach_id in eligible_machine_ids:
                    if mach_id < self.num_mas:
                        eligible_machine_loads.append(self.JSM_feat_mas_batch[0, 3, mach_id].item())
                
                if eligible_machine_loads:
                    avg_load = sum(eligible_machine_loads) / len(eligible_machine_loads)
                    self.JSM_feat_opes_batch[0, 7, op_to_update.operation_id] = avg_load
                else:
                    self.JSM_feat_opes_batch[0, 7, op_to_update.operation_id] = 0 # Default for no eligible machines

        # --- Calculate Machine Switching Penalty ---
        # Initialize to 0 for this step
        self.machine_switching_penalty = 0.0
        # Check if a switch occurred for the scheduled operation
        # ope_idx is the index of the scheduled operation
        # mac_idx is the machine it was scheduled on
        predecessor_machine_id = self.JSM_feat_opes_batch[0, 6, ope_idx].item()
        # Only penalize if there was a valid predecessor and a switch occurred
        if predecessor_machine_id != -1 and predecessor_machine_id != mac_idx:
            self.machine_switching_penalty = 1.0 # Simple penalty of 1 for each switch

        # --- Define Reward Weights (hardcoded for now) ---
        w1_makespan = 1.0 # Weight for makespan
        w2_workload_imbalance = 0.1 # Weight for workload imbalance
        w3_machine_switching = 0.5 # Weight for machine switching

        # --- Combine into new reward_batch ---
        # Current makespan reduction reward
        makespan_reward_component = self.makespan_batch - max_makespan

        # Total penalty from new objectives
        total_penalty = (w2_workload_imbalance * self.workload_imbalance_penalty) + 
                        (w3_machine_switching * self.machine_switching_penalty)

        # New reward: makespan reduction minus new penalties
        # We want to minimize makespan and penalties, so we subtract them
