...
class CaseGenerator:
    ...
    def __init__(self, job_init, num_mas, opes_per_job_min, opes_per_job_max, nums_ope=None, path='../data/',
                 flag_same_opes=True, flag_doc=False, is_dynamic=False):
        if nums_ope is None:
            nums_ope = []
    ...
    def get_case(self, idx=0):
        ...
        arrival_times = [0.0] * self.num_jobs
        if self.is_dynamic:
            # Simplified arrival time generation
            avg_proc_time = (self.proctime_per_ope_min + self.proctime_per_ope_max) / 2.0
            current_arrival_time = 0.0
            for i in range(self.num_jobs):
                inter_arrival_time = random.uniform(0, 2 * avg_proc_time)
                current_arrival_time += inter_arrival_time
                arrival_times[i] = current_arrival_time
        
        return lines, arrival_times